...
export default function AccountsERP({ theme, showToast, API, vendor }) {
  ...
  const fetchData = async () => {
    ...
    try {
      const [accRes, vchRes] = await Promise.all([
        fetch(`${API}/api/accounts/coa`, { headers: h }),
        fetch(`${API}/api/accounts/vouchers`, { headers: h })
      ]);
  ...
  const handleSaveAccount = async () => {
    if (!newAcc.name) return showToast('Name is required');
    try {
      const res = await fetch(`${API}/api/accounts/coa`, {
        method: 'POST', headers: h, body: JSON.stringify(newAcc)
      });
      if (res.ok) {
  ...
  const handleSaveVoucher = async () => {
    ...
    if (Math.abs(debits - credits) > 0.01) return showToast('Dr/Cr must be balanced');

    try {
      const res = await fetch(`${API}/api/accounts/vouchers`, {
        method: 'POST', headers: h, body: JSON.stringify(newVch)
      });
      if (res.ok) {
        showToast('Voucher Posted');
  ...
  const tabBtn = (id, label) => (
    <button 
      ...
      style={{
        ...
        color: subTab === id ? '#000' : theme.muted,
      }}
    >{label}</button>
  );
  ...
  return (
    <div className="fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <h2 style={{ margin: 0, color: theme.text }}>Cloud Accounting & Ledgers</h2>
        <div style={{ display: 'flex', gap: 12 }}>
          <button onClick={() => setIsAddingAcc(true)} style={{ background: theme.bg, color: theme.text, border: `1px solid ${theme.border}`, padding: '10px 20px', borderRadius: 12, cursor: 'pointer', fontWeight: 700 }}>+ Add COA</button>
          <button onClick={() => setIsAddingVch(true)} style={{ background: '#39FF14', color: '#000', border: 'none', padding: '10px 20px', borderRadius: 12, cursor: 'pointer', fontWeight: 800 }}>+ Post Journal</button>
        </div>
      </div>
