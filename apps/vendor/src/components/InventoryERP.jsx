...
export default function InventoryERP({ vendor, theme }) {
  ...
  const [modalType, setModalType] = useState('invoice'); // invoice, issuance, adjustment, wastage
  const [formData, setFormData] = useState({ 
    storeId: '', vendorId: vendor.id, departmentId: '', supplierId: '', 
    notes: '', reason: '', 
  ...
  const fetchData = async () => {
    ...
    const h = getHeaders();
    try {
      const [s, i, iss, l, p, o, d, sup, adj, wst] = await Promise.all([
        ...
        fetch(`${API}/api/inventory/suppliers`, { headers: h }).then(r => r.json()),
        fetch(`${API}/api/inventory/adjustments`, { headers: h }).then(r => r.json()),
        fetch(`${API}/api/inventory/wastage`, { headers: h }).then(r => r.json())
      ]);
      setData({
        ...
        adjustments: Array.isArray(adj) ? adj : [],
        wastage: Array.isArray(wst) ? wst : []
      });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    let endpoint = '';
    if (modalType === 'invoice') endpoint = 'purchase-invoices';
    else if (modalType === 'issuance') endpoint = 'issuances';
    else if (modalType === 'adjustment') endpoint = 'adjustments';
    else if (modalType === 'wastage') endpoint = 'wastage';
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(err.error || 'Operation failed. Check your data.');
      }
  ...
  const approve = async (type, id) => {
    let endpoint = '';
    if (type === 'invoice') endpoint = `purchase-invoices/${id}/approve`;
    else if (type === 'issuance') endpoint = `issuances/${id}/approve`;
    else if (type === 'adjustment') endpoint = `adjustments/${id}/approve`;
    else if (type === 'wastage') endpoint = `wastage/${id}/approve`;
  ...
  const tabSty = (active) => ({ padding: '10px 20px', borderRadius: 12, border: 'none', background: active ? '#8de02c' : 'rgba(0,0,0,0.2)', color: active ? '#000' : theme.text, cursor: 'pointer', fontWeight: 700, transition: '0.3s', display: 'flex', alignItems: 'center', gap: 8, whiteSpace: 'nowrap' });
    ...
    <div className="fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <button onClick={() => { setModalType('invoice'); setFormData({ storeId: '', vendorId: vendor.id, supplierId: '', notes: '', lines: [{ productId: '', quantity: 1, rate: 0 }] }); setShowModal(true); }} style={{ padding: '12px 20px', borderRadius: 12, border: 'none', background: '#3b82f6', color: '#fff', cursor: 'pointer', fontWeight: 800, display: 'flex', alignItems: 'center', gap: 8 }}>
            <ShoppingCart size={18} /> Purchase
          </button>
          <button onClick={() => { setModalType('issuance'); setFormData({ storeId: '', vendorId: vendor.id, departmentId: '', notes: '', lines: [{ productId: '', quantity: 1, rate: 0 }] }); setShowModal(true); }} style={{ padding: '12px 20px', borderRadius: 12, border: 'none', background: '#8b5cf6', color: '#fff', cursor: 'pointer', fontWeight: 800, display: 'flex', alignItems: 'center', gap: 8 }}>
